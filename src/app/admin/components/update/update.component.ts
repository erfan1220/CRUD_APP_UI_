...
@Component({
  selector: 'app-update',
  imports: [ModalComponent, CommonModule, FormsModule, ImagesComponent, LoadingSvgComponent, SpecificationsComponent],
...
export class UpdateComponent {
  ...
  image_src: string = ''
  specs: { categoryId: number, subCategory: string, value: string, relatedSubs: { name: string, id: number }[] }[] = []
  labels = [
    { name: 'Short Description', rows: 1, cols: 50, value: "Write your short description here..." },
    { name: 'Description', rows: 5, cols: 50, value: "Write your description here..." },
    { name: 'Expert Review', rows: 5, cols: 50, value: "Write your expert review here..." },
  ];
        ...
        if (this.detail?.explanation?.length) {
          this.detail.explanation
            ...
            .forEach(e => {
              ...
              this.exp.push({ label: e.partname, value: e.value, rows: this.rows, cols: this.cols });
            });
        } else {
          this.labels.forEach(m => {
            this.exp.push({ label: m.name, value: m.value, rows: this.rows, cols: this.cols });
          })
        }
        this.loading = false
      }
